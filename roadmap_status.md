...
- 🟡 **Stage 1b — Backstop, escrow & extend (gate).** Split into two tested parts.
  - ✅ **Part 1 — backstop lifecycle + extend (done 2026-06-14).** Shipped: `kind=backstop`/`state=dormant` pledge (no disk reservation — leans on the existing copy; `GET /backstop/quote` → pay → `POST /backstop/pledge`, replay-guarded by `payments.tx_id`); **FIFO baton-pass** promotion (`quota.reconcileCidAfterEnd`, wired into both `sweep` and `cancelClaim` — `created_ts ASC, rowid ASC`); dormant-cancel **full-refund minus `BACKSTOP_CANCEL_FEE_PCT`** (`pricing.calculateDormantRefund`, admin-forced voids pass `feePct=0`); **extend/top-up** at `rate_locked` (`quota.extendClaim`, `GET /claims/extend/quote` + `POST /claims/extend`); `GET /backstop/queue` debug view (public during testing per cohosting §9). `test/backstop-lifecycle.test.js` — 9 tests green (pledge shape, FIFO activate on expire AND cancel, FIFO order, dormant-cancel fee, no-fee admin void, extend+pin sync, guards).
  - ❌ **Part 2 — moderation × escrow (next).** CID ban = content kill + permanent banned-CID registry (checked at reserve/upload/**pledge**) + void the whole queue; user ban = identity kill + baton-pass survival; forced voids = no cancel fee, backstoppers full-refund, offender per `refund_policy`. Touches `moderation.js` + `/admin/ban` + `/admin/takedown`. *Test: a CID ban voids the queue + blocks re-upload + refunds backstoppers in full.* Full spec: [ipfs-gate-cohosting-backstop.md](ipfs-gate-cohosting-backstop.md) §7.
