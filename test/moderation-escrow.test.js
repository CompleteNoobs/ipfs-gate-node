...
// whole dormant guardian queue, registry-blocks re-upload, and classifies refunds
// (innocent guardians = full escrow; offender per refund_policy); user ban
// (identity kill) voids only that user's claims and lets the file SURVIVE via
// another user's guardian. DB-level, no Kubo/Hive/network.
//
//   node --test test/
...
function setupGuardian({ owner, cid, sizeBytes = 5_000_000, pledgedHours = 5 }) {
...
test('CID ban voids the active claim AND the whole guardian queue + blocks re-upload', () => {
  const cid = newCid();
  const foo = setupActiveClaim({ owner: u('alice'), cid });
  const bar = setupGuardian({ owner: u('bob'), cid });
  const baz = setupGuardian({ owner: u('carol'), cid });
  ...
  assert.equal(quota.getDormantGuardiansForCid(cid).length, 0);  // queue voided
...
test('CID ban refunds: innocent guardians FULL escrow; active offender pro-rata', () => {
  const cid = newCid();
  const foo = setupActiveClaim({ owner: u('alice'), cid, sizeBytes: 5_000_000, paidHours: 5, startOffsetMs: 0 });
  const bar = setupGuardian({ owner: u('bob'), cid, sizeBytes: 5_000_000, pledgedHours: 5 }); // escrow 25

  const r = moderation.takedownCid({ cid, reason: 'dmca', refund_policy: 'prorata' });
  const byId = Object.fromEntries(r.voided_claims.map(c => [c.claim_id, c]));

  // innocent guardian → full escrow, no fee
...
test('CID ban with refund_policy=none: offender forfeits, guardian still full', () => {
  const cid = newCid();
  const foo = setupActiveClaim({ owner: u('alice'), cid });
  const bar = setupGuardian({ owner: u('bob'), cid });
...
test('user ban voids the user’s claim but the file SURVIVES via another user’s guardian', () => {
  const cid = newCid();
  const alice = u('alice');
  ...
  assert.equal(quota.getClaim(foo.claim_id).state, 'cancelled');
  assert.equal(quota.getClaim(bar.claim_id).state, 'active');    // bob's guardian took the baton
...
test('user ban voids the banned user’s OWN dormant guardian (per policy, not innocent)', () => {
  const cid = newCid();
  const bob = u('bob');
  const alice = u('alice');
  setupActiveClaim({ owner: bob, cid });                         // bob keeps the file alive
  const aliceBs = setupGuardian({ owner: alice, cid, sizeBytes: 5_000_000, pledgedHours: 5 }); // escrow 25
  ...
  // banned user's own dormant guardian, prorata → full escrow (never metered); none → forfeit
